...
{
    MessageBox(0, s, TEXT("hint"), 0);
    return;
}

bool twoselection(char s[100])  //弹出有确定和取消两个选项的弹窗
{
    int a = MessageBox(0, s, TEXT("hint"), 1);  //弹窗内容取决于传过来的字符串
    if (a == 1)                                 //选择确定则返回真
        return true;
    else  //选择取消返回假
        return false;
...
void Create_Maze()  //原理是用最小生成树生成联通道路，这里用的是并查集+kruskal
{
    ...
    for (int i = 0; i < Wall_Num; i++) {
        ...
        if (bcj.Set_Union(y1 * Maze_Size + x1, y2 * Maze_Size + x2)) {  //合并了集合说明需要拆墙
...
int main() {
    ...
    if (mciSendString(TEXT("open m_Regrets.mp3 alias BGM"), NULL, 0, NULL))  //打开音乐文件
    {
        oneselection((char *)"OPEN BGM FAILED");
        return 0;
    }
    mciSendString(TEXT("play BGM"), NULL, 0, NULL);  //播放音乐
    if (MessageBox(0, TEXT("Start Game Now?"), TEXT("hint"), 1) == 2)
        return 0;
    mciSendString(TEXT("close BGM"), NULL, 0, NULL);  //在这里停止播放否则在输入时会出现音乐卡顿
    ...
    if (mciSendString(TEXT("open m_CTS.mp3 alias BGM"), NULL, 0, NULL))  //获取新的音乐文件
    {
        oneselection((char *)"OPEN BGM FAILED");
        return 0;
    }
    ...
    if (mciSendString(TEXT("open m_bgm.mp3 alias BGM"), NULL, 0, NULL))  //获取新的音乐文件
    {
        oneselection((char *)"OPEN BGM FAILED");
        return 0;
    }
    mciSendString(TEXT("play BGM repeat"), NULL, 0, NULL);                   //循环播放背景音乐
    Create_Maze();                                                           //创建迷宫
    Story_Start();                                                           //故事开始界面
    ...
    {
        if (mciSendString(TEXT("open m_ding.mp3 alias BGM"), NULL, 0, NULL))  //获取新的音乐文件
        {
            oneselection((char *)"OPEN BGM FAILED");
            return 0;
        }
        mciSendString(TEXT("play BGM"), NULL, 0, NULL);  //特殊结局特殊音效
